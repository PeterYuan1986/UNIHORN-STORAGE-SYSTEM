...
if (isset($_POST['search'])) {
    $sql = "SELECT sku, shanghai FROM product where shanghai>0 and sku LIKE '%" . @$_POST['searchtext'] . "%' ORDER BY " . $column . ' ' . $sort_order;
} else {
    $sql = "SELECT sku, shanghai FROM product where shanghai>0 ORDER BY " . $column . ' ' . $sort_order;
}
$result = mysqli_query($conn, $sql);
